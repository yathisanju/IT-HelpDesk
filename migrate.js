...
const path = 'c:\\Users\\IT-ROHL\\Desktop\\Github\\index.html';
...
html = html.replace(/\/\/ ═══════════════════════════════════════════════════════\s*\/\/ ZOHO WORKDRIVE CONFIG[\s\S]*?\/\/ ═══════════════════════════════════════════════════════\s*\/\/ STATE/, backendConfig);
...
html = html.replace(/function doLogin\(\) \{[\s\S]*?(?:return;\s*\n\s+\}\s*)+[\s\S]*?err\.textContent = '⚠️ Invalid email or password\.';\s*\}/, newLogin);

const newUpdate = `async function updateTicket(id) {
...
html = html.replace(/async function updateTicket[\s\S]*?setTimeout\(\(\)=>{ const el=document\.getElementById\('at-'\+id\); if\(el\) el\.classList\.add\('expanded'\); },50\);\s*\}/, newUpdate);

const newSubmit = `async function submitTicket(){
  if(!validate())return;
...
html = html.replace(/async function submitTicket\(\)\{[\s\S]*?(?:btn\.disabled=false; btn\.textContent='🚀 Submit Support Ticket';\s*\n\s+\}\s*)\}/, newSubmit);

const descBox = '<div class="detail-row full"><div class="d-label">Description</div><div class="desc-box">${t.desc}</div></div>';
const descReplace = descBox + '\\n          ${t.attachmentLink ? `<div class="detail-row full"><div class="d-label">Attachment</div><div class="d-val"><a href="${t.attachmentLink}" target="_blank" style="color:var(--success);text-decoration:none;">📎 Open stored file</a></div></div>` : ``}';
html = html.replace(descBox, descReplace);

const newAddAdmin = `async function addHotelAdmin(){
...
html = html.replace(/function addHotelAdmin\(\)\{[\s\S]*?renderAdminUsers\(\);\s*\}/, newAddAdmin);

const newDelAdmin = `async function deleteAdmin(idx){
  if(currentUser.role!=='superadmin')return;
...
html = html.replace(/function deleteAdmin\(idx\)\{[\s\S]*?renderAdminUsers\(\);\s*\}/, newDelAdmin);
